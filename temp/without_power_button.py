...
power_button= html.Div([ daq.PowerButton(on=True,color=on,className='dark-theme-control',size=150,id='power_button',
...
app.layout=html.Div([ dbc.Row([db_header_text],style=dict(backgroundColor='#20374c') ),html.Br(),
       dbc.Row([
        ...
        , style=dict(backgroundColor='#20374c')), html.Br()
         ...
         ,
           ...
           dbc.Col([
               ...
               , html.Br()
           ], xl=dict(size=2, offset=5), lg=dict(size=2, offset=5),
               md=dict(size=5, offset=1), sm=dict(size=6, offset=0), xs=dict(size=6, offset=0),
               style=dict(display='inline-block', verticalAlign='middle')
           )
        ...
        daq.DarkThemeProvider(theme=theme, children=power_button)
    ], style={
                                   })
...
@app.callback([Output('presoaking_indicator','color'),Output('soap_indicator','color'),
               Output('degreaser_indicator','color'),Output('rinse_indicator','color'),Output('wax_indicator','color'),
              ...
              [Input('presoaking_button','n_clicks'),Input('soap_button','n_clicks'),
               Input('degreaser_button','n_clicks'),Input('rinse_button','n_clicks'),Input('wax_button','n_clicks'),
                Input('pressure_washer_button','n_clicks'),Input('heater_button','n_clicks'),Input('emergency_stop','n_clicks')
               ],
               ...
               State('pressure_washer_indicator','color'),State('heater_indicator','color')
...
def cycle_control(presoaking_button,soap_button,degreaser_button,
    ...
    if system_state==idle_state:
        if button_pressed=='pressure_washer_button':
            if pressure_washer_state==off:
                system_state=washing_state
                start_time=time.time()
                # pressure_washer pin on
                return (dash.no_update,dash.no_update,dash.no_update,dash.no_update,dash.no_update,on,dash.no_update)
            else:
                raise PreventUpdate
        ...
        elif button_pressed=='pressure_washer_button':
            if pressure_washer_state == on :
                system_state=finished_washing_state
                # all pins off
                return (off,off,off,off,off,off,off)
