...
presoaking_on_button=html.Div([dbc.Button("Turn On",size='lg',outline=True, color="success", className="me-1", n_clicks=0,id="presoaking_on_button"
...
cycle_timer=daq.LEDDisplay(
    ...
    value='120' , size=60 , color='#42C4F7',backgroundColor="#0f2937"
#42C4F7"
#0f2937 #FF5E5E
)
...
def cycle_control(on_button,off_button,pw_indicator):
    ...
    if system_state==washing_state:
        ...
        if button_pressed=='presoaking_off_button':
...
@app.callback(Output('soap_indicator','color'),
              [Input('soap_on_button','n_clicks'),Input('soap_off_button','n_clicks')]
              ,prevent_initial_call=True)
def soap_control(on_button,off_button):
    ctx = dash.callback_context
    button_pressed=ctx.triggered[0]['prop_id'].split('.')[0]
    if button_pressed=='soap_off_button':
...
@app.callback(Output('degreaser_indicator','color'),
              [Input('degreaser_on_button','n_clicks'),Input('degreaser_off_button','n_clicks')]
              ,prevent_initial_call=True)
def degreaser_control(on_button,off_button):
    ctx = dash.callback_context
    button_pressed=ctx.triggered[0]['prop_id'].split('.')[0]
    if button_pressed=='degreaser_off_button':
...
@app.callback(Output('rinse_indicator','color'),
              [Input('rinse_on_button','n_clicks'),Input('rinse_off_button','n_clicks')]
              ,prevent_initial_call=True)
def rinse_control(on_button,off_button):
    ctx = dash.callback_context
    button_pressed=ctx.triggered[0]['prop_id'].split('.')[0]
    if button_pressed=='rinse_off_button':
...
@app.callback(Output('pressure_washer_indicator','color'),
              [Input('pressure_washer_on_button','n_clicks'),Input('pressure_washer_off_button','n_clicks')]
              ,prevent_initial_call=True)
def pressure_washer_control(on_button,off_button):
    ctx = dash.callback_context
    button_pressed=ctx.triggered[0]['prop_id'].split('.')[0]
    if button_pressed=='pressure_washer_off_button':
...
@app.callback(Output('heater_indicator','color'),
              [Input('heater_on_button','n_clicks'),Input('heater_off_button','n_clicks')]
              ,prevent_initial_call=True)
def heater_control(on_button,off_button):
    ctx = dash.callback_context
    button_pressed=ctx.triggered[0]['prop_id'].split('.')[0]
    if button_pressed=='heater_off_button':
