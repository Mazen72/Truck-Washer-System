...
with open('logo.json', 'r') as openfile:
...
df_colors = pd.read_csv('colors.csv')  # get coordinates data
...
cycle_timer=daq.LEDDisplay(
    ...
    value='0' , size=42 , color=components_colors['Timer Numbers'][0],backgroundColor=components_colors['Timer Background'][0]
#42C4F7"
#0f2937 #FF5E5E
)
        ...
        , style=dict(backgroundColor=components_colors['Containers Background'][0]),id='card1',className='mycard'), html.Br()
                     ...
                     md=dict(size=3,offset=0),sm=dict(size=6,offset=0),xs=dict(size=6,offset=0)),
                      ...
                      dbc.Col([dbc.Card(dbc.CardBody(
                          ...
                          , style=dict(backgroundColor='#20374c'),id='card2',className='mycard'), html.Br()
         ...
         ,
           ...
           dbc.Col([
               ...
               md=dict(size=6, offset=3), sm=dict(size=6, offset=3), xs=dict(size=6, offset=3),
               style=dict()
           )
...
if system_mode=='editing':
    app.layout=html.Div([  main_layout, dbc.Row([
        dbc.Col([dbc.Card(dbc.CardBody(
        ...
        ], xl=dict(size=4, offset=4), lg=dict(size=6, offset=3),
            md=dict(size=8, offset=2), sm=dict(size=10, offset=1), xs=dict(size=10, offset=1), style=dict()
        )
...
if system_mode!='editing':
    @app.callback([Output('presoaking_indicator', 'value'), Output('soap_indicator', 'value'),
                  ...
                  [Input('presoaking_button', 'n_clicks'), Input('soap_button', 'n_clicks'),
                   ...
                   Input('power_button', 'on'), Input('heater_button', 'n_clicks')
                   ],
    ...
    def cycle_control(presoaking_button, soap_button, degreaser_button,
        ...
        if system_state == idle_state:
            ...
            if button_pressed == 'power_button':
                ...
                # pressure_washer pin on
                return (
                dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, True, dash.no_update)
        else:
            raise PreventUpdate
...
if system_mode!='editing':
    ...
    def update_timer(time_update,power_button):
        global seconds , minutes , time_passed
    ...
    def update_logo(list_of_contents, filename):
        ...
        try:
            if ('jbg' in filename) or ('jpeg' in filename ) or ('png' in filename) or ('svg' in filename):
                ...
                with open("logo.json", "w") as outfile:
